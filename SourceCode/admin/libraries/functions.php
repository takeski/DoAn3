...
function ck_per($com, $act)
{
	global $d;
	
	if (!isset($_SESSION["accesstokens"]) || !isset($_SESSION["defaultaccesstoken"])) {
		return false;
	}
	
	$accesstokens = json_decode($_SESSION["accesstokens"], true);
	foreach ($accesstokens as $token) {
		if ($token["type"] == $com) {
			$accessgrant = json_decode($token["accessgrant"], true);
			$accessmask = json_decode($token["accessmask"], true);
			
			$check = false;
			switch ($act) {
				case "r":
					$check = ($accessgrant[0] - $accessmask[0]) >= 0;
					break;
				case "c":
					$check = ($accessgrant[1] - $accessmask[1]) >= 0;
					break;
				case "u":
					$check = ($accessgrant[2] - $accessmask[2]) >= 0;
					break;
				case "d":
					$check = ($accessgrant[3] - $accessmask[3]) >= 0;
					break;
	...
	$defaultaccesstoken = $_SESSION["defaultaccesstoken"];
	$accessgrant = json_decode($token["defaultaccesstoken"], true);
	
	foreach ($managementobjects as $mo) {
		if ($mo["type"] == $com) {
			$accessmask = json_decode($mo["accessmask"], true);
...
function GetTypeName($type) {
	...
	$d->SetWhere("parentid", "=", $_GET["pid"]);
	$d->SetWhere("link", "=", $type);
	return reset($d->Select("name"))["name"];
}
...
function SetWatermark($img)	
{
	...
	$d = new Query();
	$d->SetTable("#_companyinfo");
	$stamp = SERVER_ROOT . reset($d->Select("stamp"))["stamp"];
	$uploadimage = SERVER_ROOT . $img;

	$actual = SERVER_ROOT . $img;

	// Load the main image
	$source = imagecreatefromjpeg($uploadimage);

	// load the image you want to you want to be watermarked
	$watermark = imagecreatefrompng($stamp);
...
function get_host_name($pid){
	global $d, $row,$lang;
	$sql = "select ten from #_host where id='".$pid."'";
	$d->query($sql);
	$row = $d->fetch_array();
	return $row['ten'];
}
	
function get_host_price($pid){
	global $d, $row,$lang;
	$sql = "select gia from #_host where id='".$pid."'";
	$d->query($sql);
	$row = $d->fetch_array();
	return $row['gia'];
}
...
function get_tong_tien($id=0){
		...
		if($id>0){
			$d->reset();
			$sql="select total from #_donhang where id='".$id."'";
			$d->query($sql);
			$total= reset($d->result_array())["total"];
			/*$tongtien=0;
...
function upload_image($file, $extension, $folder, $newname=''){
	...
	if(isset($_FILES[$file]) && !$_FILES[$file]['error']){
		...
		if (!copy($_FILES[$file]["tmp_name"], $folder.$_FILES[$file]['name']))	{
			echo $folder.$_FILES[$file]['name']; exit(0);
