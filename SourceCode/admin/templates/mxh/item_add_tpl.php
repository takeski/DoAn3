<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="<?=CUR_PATH?>?com=mxh&act=man_slides<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"><span>Mạng xã hội</span></a></li>
            <li class="current"><a href="#" onclick="return false;"><?= isset($_GET["id"]) ? "Sửa mạng xã hội" : "Thêm mạng xã hội" ?></a></li>
        </ul>
        <div class="clear"></div>
    </div>
</div>
...
<form name="supplier" id="validate" class="form" action="<?=CUR_PATH?>?com=mxh&act=save<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
        <?php// for($i=0; $i<1; $i++){?>
        <div class="title"><img src="<?=CUR_PATH?>/images/icons/dark/list.png" alt="" class="titleIcon" />
			<h6><?= isset($_GET["id"]) ? "Sửa mạng xã hội" : "Thêm mạng xã hội" ?></h6>
		</div>
		<div class="formRow">
			<label>Tên:</label>
			<div class="formRight">
