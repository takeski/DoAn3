...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
            ...
            <li class="current"><a href="#" onclick="return false;"><?= isset($_GET["id"]) ? "Sửa quảng cáo" : "Thêm quảng cáo" ?></a></li>
        </ul>
        <div class="clear"></div>
    </div>
</div>
...
<form name="supplier" id="validate" class="form" action="<?=CUR_PATH?>?com=bannerqc&act=save_bannerqc<?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
	<div class="widget">
				
		
        <?php// for($i=0; $i<1; $i++){?>
        <div class="title"><img src="<?=CUR_PATH?>/images/icons/dark/list.png" alt="" class="titleIcon" />
			<h6><?= isset($_GET["id"]) ? "Sửa quảng cáo" : "Thêm quảng cáo" ?></h6>
		</div>
		<div class="formRow">
			<label>Tên:</label>
			<div class="formRight">
		...
		<div class="formRow">
			<label>Đặt ở:</label>
			<div class="formRight">
                <select id="bannerlocation" name="bannerlocation">
					<option value="left" <?php echo $item["loai"] == "left" ? "selected='selected'" : "" ?>>Bên trái</option>
					<option value="right" <?php echo $item["loai"] == "right" ? "selected='selected'" : "" ?>>Bên phải</option>
				</select>
			</div>
			<div class="clear"></div>
		</div>		
