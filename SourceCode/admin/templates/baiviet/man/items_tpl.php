...
<div class="widget">
  <div class="title">
    ...
    <thead>
      <tr>
        ...
        <?php if ($_GET['type']== "tintuc"){?>
          <td class="sortCol"><div>Hình</div></td>  
        <?php }?>
        <td class="sortCol"><div>Tên bài viết</div></td>   	
    ...
    <tbody>
        ...
        <?php if ($_GET['type']== "tintuc"){?>
          <td><div> <img style="width: 140px;height: 70px;" src=" <?= WEB_ROOT . $item['hinh']?>" alt=""> </div></td>  
        <?php }?>	
				<td align="center">
					<a href="<?=CUR_PATH?>?com=<?=$com?>&act=edit&id=<?=$item['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" class="tipS SC_bold"><?=$item['ten']?></a>
				</td>
			
				<td class="actBtns">
					<a href="<?=CUR_PATH?>?com=<?=$com?>&act=edit&id=<?=$item['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" title="" class="smallButton tipS" original-title="Sửa bài viết"><img src="<?=CUR_PATH?>/images/icons/dark/pencil.png" alt=""></a>
					<a href="<?=CUR_PATH?>?com=<?=$com?>&act=delete&id=<?=$item['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" onClick="if(!confirm('Xác nhận xóa')) return false;" title="" class="smallButton tipS" original-title="Xóa bài viết"><img src="<?=CUR_PATH?>/images/icons/dark/close.png" alt=""></a>
				</td>
			</tr>
        <?php } ?>
	</tbody>
